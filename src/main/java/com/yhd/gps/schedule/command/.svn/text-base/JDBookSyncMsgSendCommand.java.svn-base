 ...
 */
public class JDBookSyncMsgSendCommand extends ShardingDataExecCommand<List<Integer>, List<JDBookMessageVo>> {
    private Integer msgSize;
    private JDBookSyncService jdBookSyncService;

    public JDBookSyncMsgSendCommand(int shardingIndex, CountDownLatch finishSignal, String bussinessType,
    ...
    public List<JDBookMessageVo> fetchBussinessDataes(int shardingIndex) {
        Long offset = 0L;
        // 查询商家id
        List<Long> merchantIds = MiscConfigUtils.getConfigListByItemKey(ScheduleConstants.JD_BOOK_SYNC_MERCHANTID_LIST);
